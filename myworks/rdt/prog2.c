...
int checksum(int calling_entity, struct pkt packet)
{
  int sum = 0;
  sum += packet.seqnum;
  sum += packet.acknum;
...
void array_copy(char dest[20], char source[20])
{
  for(int i = 0; i < 20; i++)
  {
    dest[i] = source[i];
...
{
  if(A_info.a_status == WAIT)
  {
    printf("A无法发送，正在等待ack\n");
    return;
  }
  struct pkt outpkt;
  outpkt.acknum = A_info.waitacknum;
  array_copy(outpkt.payload, message.data);
  outpkt.seqnum = A_info.nextseqnum;
  A_info.nextseqnum = 1 - A_info.nextseqnum;
  A_info.waitacknum = 1 - A_info.waitacknum;
  outpkt.checksum = checksum(0, outpkt);
  A_info.lastpkt = outpkt;
  A_info.a_status = WAIT;
  starttimer(0, 20.0);
  tolayer3(0, outpkt);
  printf("A发送一个新数据包: seqnum: %d, checksum: %d\n", outpkt.seqnum, outpkt.checksum);
}
...
{
  int cs = checksum(1, packet);
  if(cs != 0)
  {
    printf("A收到一个破损的数据包, cs: %d\n", checksum(0, packet));
    return;
  }
  int acknum = packet.acknum;
  if(acknum == A_info.waitacknum)
  {
    printf("A成功ack: acknum: %d, A的上一次发包的seqnum: %d\n", acknum, A_info.lastpkt.seqnum);
    A_info.a_status = READY;
    stoptimer(0);
    return;
  }
  printf("acknum不正确: 期待收到的acknum: %d, 收到的acknum: %d\n", A_info.waitacknum, acknum);
}

/* called when A's timer goes off */
void A_timerinterrupt()
{
  printf("A等待超时，最近发送的包的seqnum: %d\n", A_info.lastpkt.seqnum);
  tolayer3(0, A_info.lastpkt);
  starttimer(0, 20.0);
}  
...
void B_input(struct pkt packet)
{
  ...
  int cs = checksum(1, packet);
  char fill[20] = {0};
  array_copy(outpkt.payload, fill);
  outpkt.seqnum = packet.acknum;
  if(cs != 0){
    printf("B收到一个破损的数据包\n");
    outpkt.acknum = B_info.expectednum;
    outpkt.checksum = checksum(0, outpkt);
    printf("B重新发送上一次的acknum: %d\n", outpkt.acknum);
    tolayer3(1, outpkt);
    return;
  }
  if(packet.seqnum == B_info.expectednum){
    outpkt.acknum = 1 - B_info.expectednum;
    B_info.expectednum = 1 - B_info.expectednum;
    outpkt.checksum = checksum(0, outpkt);
    tolayer3(1, outpkt);
    array_copy(inmsg.data, packet.payload);
    tolayer5(inmsg.data);
    printf("B收到正确数据包，数据包seqnum: %d, B发送的acknum: %d", packet.seqnum, outpkt.acknum);
    return;
  }
  printf("B收到的数据包的acknum与期待的acknum不对应, 数据包seqnum: %d, 期待seqnum: %d\n", packet.seqnum, B_info.expectednum);
  outpkt.acknum = B_info.expectednum;
  outpkt.checksum = checksum(0, outpkt);
  printf("B重新发送上一次的acknum: %d\n", outpkt.acknum);
  tolayer3(1, outpkt);
